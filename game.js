  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Because there is a third case (space ' '), use an "else if" instead of "else"
        ...
        fieldType = "lava";
      // this is for the fireball obstacle
      else if (ch == "y")
        fieldType = "fireball";
      else if (ch == "B")
      fieldType = "floater"
...
function Staff(pos) {
  ...
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobb = Math.random() * Math.PI * 2;
}
...
Staff.prototype.act = function(step) {
  ...
  //need to figure out why line 318 & 319 wont work when Un comentted
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "fireball" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "moon" || type == "Staff") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any moons left, player wins
